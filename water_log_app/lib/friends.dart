...
class friends extends StatefulWidget {
  ...
  friends({Key? key, required this.client}) : super(key: key);
...
class _friends extends State<friends> {
  ...
  // Lists to contain friends and the requests
  ...
  Widget build(BuildContext context) {
    // Builds the widget for the friends system
    final appbarHeight = appbar.preferredSize.height;
    return Scaffold(
        appBar: appbar,
        backgroundColor: Colors.white,
        body: FutureBuilder<List<List<friendEntity>>>(
            future: Future.wait([
              getFriends(widget.client.userName),
              getRequests(widget.client.userName),
              // API metho calls to get data
            ]),
            builder: (
                          ...
                          child: ListView(
                            children: <Widget>[
                              ...
                              FlatButton(
                                ...
                                onPressed: () async {
                                  _controller.clear();
                                  _showAlertDialog(await sendRequest(
                                      widget.client.userName, _name));
                                },
                                child: Text(
                                  "Add Friend",
                                  style: TextStyle(
...
Future<List<friendEntity>> getFriends(String uName) async {
  // Future Builder waits for API to be called and then loads page
  final response =
  ...
  if (response.statusCode == 200) {
    ...
    for (int i = 0; i < jsonDecode(response.body).length; i++) {
      ...
      if (reqed == uName) {
        friendList
            .add(friendEntity(reqee, jsonDecode(response.body)[i]['FS_ID']));
      } else {
        friendList
            .add(friendEntity(reqed, jsonDecode(response.body)[i]['FS_ID']));
      }
    }
...
Future<List<friendEntity>> getRequests(String uName) async {
  ...
  if (response.statusCode == 200) {
    ...
    for (int i = 0; i < jsonDecode(response.body).length; i++) {
      pendingFriendList.add(friendEntity(
          jsonDecode(response.body)[i]['REQUESTEE'],
...
Future<String> sendRequest(String client, String newFriend) async {
  ...
  if ("${newFriend[0].toUpperCase()}${newFriend.substring(1)}" ==
      "${client[0].toUpperCase()}${client.substring(1)}") {
...
Future acceptFriendship(int fID) async {
  // Updated request for the friend requests
...
Future deleteReq(int fID) async {
  // Delete API request for Friends System
