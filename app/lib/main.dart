...
void post() async {
	print("Made it to method");
	  final response = await http.post("http://192.168.0.174/MealTimeServer/config.php", body: {
	    "name": name.text,
	    "email": email.text,
 ...
 void validateTextField(String userInput) {
   ...
   if (userInput.length >= 8 && (userInput.contains(new RegExp(r'[!@#$%^&*(),.?":{}|<>]')))){
     setState(() {
         validPassword = false;
       });
   } else  {
            ...
            children: <Widget>[
              ...
              TextField(   
                controller: password,       
                  ...
                  decoration: InputDecoration(
                  errorText: validPassword ? 'Please enter a valid password' : null,
                  labelText: 'Password',
